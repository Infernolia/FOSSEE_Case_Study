...
df1$date1 <- 2000-00-00
for(i in 1:length(rownames(df1)))
{
  df1$date1[i] <-paste(paste(df1$year[i],substrRight(paste("0",df1$month[i], sep =""),2), sep ="-"),substrRight(paste("0",df1$day[i], sep =""),2), sep ="-")
}
...
write.csv(df1,"casestudy_final_strains_data.csv")
...
write.csv(df,"casestudy_final_cases_data.csv")