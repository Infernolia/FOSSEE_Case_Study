...
def estimateFM(I, B, H, M=None, F=None, F_T=None, M_T=None, state=None, params=None):
    ...
    for iter in range(params.maxiter):
        ...
        if params.alpha_f > 0 and params.beta_f > 0:
            val_x = fdx + ax
            val_y = fdy + ay
            shrink_factor = lp_proximal_mapping(np.sqrt(val_x ** 2 + val_y ** 2), params.alpha_f / params.beta_f,
                                                params.lp)  # isotropic "TV"
        ...
        ## F,M step
        rhs1 = rhs_f + params.beta_f * (Dx.T @ (vx - ax) + Dy.T @ (vy - ay)) + params.beta_fm * (
                    vf - af)  # f-part of RHS
        rhs2 = rhs_m + params.beta_f * (Dx.T @ (vx_m - ax_m) + Dy.T @ (vy_m - ay_m)).flatten('F') + params.beta_fm * (
                    vm - am)  # m-part of RHS
        ...
        def estimateFM_cg_Ax(fmfun0):
            ...
            ym = np.real(
                ifft2(HT * fft2(np.sum(B * (bHM - np.real(ifft2(HF, axes=(0, 1)))), 2), axes=(0, 1)), axes=(0, 1)))
            ...
            res = np.c_[yf, ym] + beta_tv4 * (
                        DTD @ fmfun) + params.beta_fm * fmfun  # common regularizers/identity terms
            return res.flatten('F')

        A = scipy.sparse.linalg.LinearOperator((4 * f.shape[0], 4 * f.shape[0]), matvec=estimateFM_cg_Ax)
        fm, info = scipy.sparse.linalg.cg(A, np.c_[rhs1, rhs2].flatten('F'), np.c_[f, m].flatten('F'), params.cg_tol,
                                          params.cg_maxiter)
        ...
        if params.verbose:
            if params.do_cost:  # not fully implemented for all terms, e.g. lambda_R
                ...
                Me[idx_m, idy_m] = m
                err = np.sum(np.reshape(
                    np.real(ifft2(fH[:, :, np.newaxis] * fft2(Fe, axes=(0, 1)), axes=(0, 1))) - B * np.real(
                        ifft2(fH * fft2(Me)))[:, :, np.newaxis] - (I - B), (-1, 1), 'F') ** 2)
                ...
                print("FM: iter={}, reldiff=({}, {}), err={}, cost={}".format(iter, np.sqrt(rel_diff2_f),
                                                                              np.sqrt(rel_diff2_m), err, cost))
            else:
...
def estimateH(I, B, M, F, Hmask=None, state=None, params=None):
    ...
    for iter in range(params.maxiter):
        ...
        def estimateH_cg_Ax(hfun):
            He[Hmask_small] = hfun
            FH = fft2(He, axes=(0, 1))
            Fh = iF * np.repeat(FH[:, :, np.newaxis], 3,
                                axis=2)  ## apply forward conv (->RGB image, summation over angles)
            BMh = B * np.repeat(np.real(ifft2(iM * FH, axes=(0, 1)))[:, :, np.newaxis], 3, axis=2)
            Fh_BMh = Fh - fft2(BMh, axes=(0, 1))
            res = np.sum(np.real(
                ifft2(np.conj(iF) * Fh_BMh - np.conj(iM3) * fft2(B * np.real(ifft2(Fh_BMh, axes=(0, 1))), axes=(0, 1)),
                      axes=(0, 1))), 2)
...
def project2pyramid(m, f, eps):
    ...
    normal = np.array([-1, eps]) / np.sqrt(
        1 + eps ** 2)  # dividing plane normal vector (for projection to oblique planes)
...
def lp_proximal_mapping(val_norm, amount, p):
    ...
    if p == 1:  ## soft thresholding
        ...
        shrink_factor[nz] = (2 / 3 * val_norm[nz] * (
                    1 + np.cos(2 / 3 * np.acos(-3 ** (3 / 2) / 4 * amount * val_norm[nz] ** (-3 / 2))))) / (
                            val_norm[nz])
    else:
